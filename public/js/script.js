...
const reservedSeats = ["A1","B6","C2","D5","E3","F6"];
...
function createSeatMap(reservedIds) {
  const rows = ['A','B','C','D','E','F'];
  const cols = 6;
  let html = '<div class="seat-map"><div class="screen">Black/White Board</div><div class="seat-grid">';
  rows.forEach(row => {
    for (let col = 1; col <= cols; col++) {
      const id = `${row}${col}`;
      const cls = reservedIds.includes(id) ? 'seat reserved' : 'seat';
      html += `<div class="${cls}">${id}</div>`;
    }
  });
  html += '</div><div class="legend"><span class="seat-legend"></span> Available <span class="seat-legend reserved"></span> Reserved</div></div>';
  return html;
}

function displaySlots(lab, date, slots) {
  ...
  if (slots.length === 0) {
    slotsContainer.innerHTML = "<p>No slots available for the selected time range.</p>";
  ...
  slots.forEach((slot, index) => {
    ...
    slotCard.innerHTML = `
      ...
      <button class="btn btn-small" onclick="reserveSlot('${lab}', '${date}', '${slot.start}', '${slot.end}')">Reserve</button>
    `;
...
function reserveSlot(lab, date, startTime, endTime) {
  // for modal
  ...
  info.textContent = `${lab} | ${date} | ${startTime} - ${endTime}`;

  // interactive seat map
  const seatContainer = document.getElementById('reserveSeatMap');
  ...
  rows.forEach(row => {
    for (let col = 1; col <= cols; col++) {
      ...
      seat.className = `searched-seat${reservedSeats.includes(id)?' searched-reserved':''}`;
      seat.textContent = id;
      if (!reservedSeats.includes(id)) {
        seat.addEventListener('click', () => {
          // toggle selected seat
          document.querySelectorAll('#reserveSeatMap .searched-seat.selected').forEach(s => s.classList.remove('selected'));
...
<div id="reserveModal" class="modal">
  <div class="modal-card-reserve">
    <h3>Choose Seat</h3>
    <p id="reserveInfo"></p>
    <div class="searched-seat-map">
      <div class="screen">Black/White Board</div>
      <div id="reserveSeatMap" class="searched-seat-grid" style="margin-bottom:20px;"></div>
    </div>
    <div class="modal-actions">
      <button id="confirmReserve" class="btn btn-small">Confirm</button>
      <button id="cancelReserve" class="btn btn-small">Cancel</button>
    </div>
  </div>
</div>
  `;
  ...
  document.getElementById('confirmReserve').addEventListener('click', () => {
    const selected = document.querySelector('#reserveSeatMap .searched-seat.selected');
    if (!selected) {
      alert('Please select a seat first.');
      return;
    }
    alert(`Seat ${selected.textContent} reserved!`);
    document.getElementById('reserveModal').style.display = 'none';
    // to be added (Update DB)
  });
}
